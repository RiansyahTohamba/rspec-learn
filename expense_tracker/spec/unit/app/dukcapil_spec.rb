    ...
    describe 'POST /dukcapil/fr_check' do
        context 'when dukcapil successfully sent respond' do
            it 'returns the score of searching result' do
                ...
                pending 'test juga apakah log dukcapil nya berhasil tersimpan atau belum'
        ...
        context 'when dukcapil cannot identify debitur' do
            it 'respond with NIK is not found' do
                response_body = { 'content' => {'status_code'=> '4000','error_message' => 'NIK Tidak ditemukan'}}
                # expect(result.data).to include('error_message')
            end
        end
    end
